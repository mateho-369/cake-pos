...
import { duration, money } from '../../lib/money'
...
export default function SalePage() {
  ...
  if (!shift) return <OpenShiftModal name={me.name} />
  ...
  const confirm = (method: PaymentMethod, cashTendered?: number) => {
    try {
      const order = db.checkout(cart, method, cashTendered)
      setCart([])
      setPayOpen(false)
      setMobileCart(false)
    ...
    <div className="app-shell flex flex-col">
      <header className="glass relative mx-3 mt-3 flex items-center gap-3 rounded-[22px] px-3 py-2.5 sm:mx-4 sm:px-4">
        ...
        <div className="min-w-0 sm:hidden">
          <p className="truncate text-sm font-semibold">{me.name.split(' ')[0]}</p>
        ...
        {me.role === 'admin' && (
          <Link to="/admin" className="hidden text-xs font-semibold sm:inline" style={{ color: 'var(--pink-deep)' }}>
            Admin
          </Link>
        )}
        <button
      ...
      <AddProductSheet open={addOpen} onClose={() => setAddOpen(false)} categories={categories} />
      <PaymentSheet
        open={payOpen}
        total={subtotal}
