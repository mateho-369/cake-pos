  ...
  return (
    <div className="app-shell relative grid place-items-center overflow-hidden px-5">
      ...
      >
        ...
        {mode === 'pin' ? (
          <PinPad value={pin} onChange={setPin} onComplete={submitPin} disabled={busy} />
        ) : (
        ...
        <p className="mt-6 text-center text-[0.72rem] leading-relaxed" style={{ color: 'var(--ink-4)' }}>
          ...
          <br />
          <Link to="/admin/login" className="underline decoration-pink-300">
            Admin Control
          </Link>
          {' · '}no Telegram auto-login
        </p>
      </motion.div>
    </div>
  )
