...
function load(): DbState {
  try {
    ...
    if (!parsed?.employees?.length || !parsed?.products?.length) return createSeed()
    return parsed
  } catch {
    return createSeed()
  }
...
export const db = {
  ...
  me(): Employee | null {
    ...
    return state.employees.find((e) => e.id === state.session?.employeeId) ?? null
  },
  ...
  loginPin(pin: string) {
    ...
    mutate((s) => {
      s.session = { employeeId: employee.id }
    })
    return employee
  },
  ...
  openShift(openingCash: number) {
    ...
    if (db.activeShift()) throw new Error('A shift is already open.')
    const shift: Shift = {
      id: uid('sh'),
      cashierId: me.id,
      cashierName: me.name,
  ...
  activeShift(employeeId?: string): Shift | null {
    const id = employeeId ?? state.session?.employeeId
    if (!id) return null
    return state.shifts.find((sh) => sh.cashierId === id && !sh.closedAt) ?? null
  },
