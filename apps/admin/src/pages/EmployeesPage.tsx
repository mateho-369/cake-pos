...
import { api, type AuthUser, type Role } from '@bloom/shared'
...
export default function EmployeesPage() {
  ...
  const reload = () => api.employees.list().then(setEmployees)
    ...
    <div className="bloom-in mx-auto max-w-5xl pb-10">
            ...
            <div className="mt-4 flex items-center justify-between">
              ...
              <label className="flex items-center gap-2 text-sm">
                ...
                <input
                  ...
                  checked={e.active}
                  onChange={(ev) => void api.employees.update(e.id, { active: ev.target.checked }).then(reload)}
                />
              </label>
            </div>
            {e.pinCode && (
      ...
      {open && (
        <div className="fixed inset-0 z-40 grid place-items-center px-4" style={{ background: 'rgba(59,10,31,0.28)' }} onClick={() => setOpen(false)}>
          <form className="sheet w-full max-w-md rounded-[24px] p-5" onClick={(e) => e.stopPropagation()} onSubmit={(e) => void submit(e)}>
            ...
            {error && (
              <p className="mt-3 text-sm" style={{ color: '#BE123C' }}>
                {error}
              </p>
            )}
