...
check(
  ...
  $$('.report-detail-table thead th').length === 8,
...
check(
  'columns: date/time, order, source, customer/cashier, items, payment, total, status',
  ['Date & time', 'Order', 'Source', 'Customer / cashier', 'Items', 'Payment', 'Total', 'Status'].every(
    (label, index) => headers[index] === label,
  ),
  headers.join(' | '),
)
