...
return new class extends Migration {
    public function up(): void
    {
        Schema::table('products', function (Blueprint $t) {
            $t->unsignedInteger('reserved_stock')->default(0)->after('stock');
        });
        ...
        Schema::create('order_payments', function (Blueprint $t) {
            ...
            $t->foreign('order_id')
                ...
                ->on('orders')
                ->restrictOnDelete();
            $t->string('method', 64)->index();
            $t->string('status', 16)->index();
            $t->unsignedBigInteger('amount_usd_cents');
            $t->unsignedInteger('exchange_rate_khr_per_usd');
            $t->unsignedBigInteger('tendered_usd_cents')->nullable();
            $t->unsignedBigInteger('tendered_khr')->nullable();
            $t->unsignedBigInteger('change_usd_cents')->nullable();
            $t->unsignedBigInteger('change_khr')->nullable();
            $t->bigInteger('settlement_rounding_khr')->default(0);
            $t->string('external_reference')->nullable();
            $t->foreignId('confirmed_by_employee_id')
                ->nullable()
